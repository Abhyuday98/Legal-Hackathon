        ...
        while results:
            for i in intents['intents']:
                ...
                if i['tag'] == results[0][0]:
                    ...
                    if 'context_set' in i:
                        if show_details: 
                            CONTEXT= str('context:', i['context_set'])
...
cursorUrls.execute(('insert into conversation_history(question, answer, context, tags) values("{}","{}","{}","{}");').format(q[0],answer, CONTEXT, TAGS)).commit()

#file.write(response(data))

#classify('is your shop open today?')
#response('shop is open yesterday?')
